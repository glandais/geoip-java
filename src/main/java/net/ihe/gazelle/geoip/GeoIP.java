 ...
 * 
 * <pre>
 * // First, create a GeoIP instance with the location of the database.
 * GeoIP lookupService = new GeoIP(&quot;c:\\geoip.dat&quot;);
 ...
 * </pre>
 * 
 * In general, a single GeoIP instance should be created and then reused repeatedly.
 * <p>
 * 
 ...
 * application. The following code snippet demonstrates how to create a GeoIP using a database that can be found on the classpath:
 * 
 * <pre>
 * String fileName = getClass().getResource(&quot;/GeoIP.dat&quot;).toExternalForm().substring(6);
 * GeoIP lookupService = new GeoIP(fileName);
 * </pre>
 * 
...
public class GeoIP {
	...
	private static GeoIP COUNTRY_LOOKUPSERVICE = null;
	private static GeoIP LOCATION_LOOKUPSERVICE = null;

	static GeoIP getLookupService(String dbName) {
		GeoIP lookupService = null;
		try {
			InputStream countryDBGZStream = GeoIP.class.getResourceAsStream("/" + dbName);
			GZIPInputStream countryDBStream = new GZIPInputStream(countryDBGZStream);

			ByteArrayOutputStream bos = new ByteArrayOutputStream();
			copy(countryDBStream, bos);
			countryDBStream.close();
			bos.close();

			lookupService = new GeoIP(bos.toByteArray());
		} catch (Exception e) {
			throw new IllegalStateException("Failed to load " + dbName);
		}
		return lookupService;
	}

	static GeoIP getCountryLookupService() {
		if (COUNTRY_LOOKUPSERVICE == null) {
			synchronized (GeoIP.class) {
				if (COUNTRY_LOOKUPSERVICE == null) {
					COUNTRY_LOOKUPSERVICE = getLookupService("GeoIP.dat.gz");
				}
			}
		}
		return COUNTRY_LOOKUPSERVICE;
	}

	public static Country getCountry_(String str) {
		return getCountryLookupService().getCountry(str);
	}

	static GeoIP getLocationLookupService() {
		if (LOCATION_LOOKUPSERVICE == null) {
			synchronized (GeoIP.class) {
				if (LOCATION_LOOKUPSERVICE == null) {
					LOCATION_LOOKUPSERVICE = getLookupService("GeoLiteCity.dat.gz");
				}
			}
		}
		return LOCATION_LOOKUPSERVICE;
	}

	public static Location getLocation_(String str) {
		return getLocationLookupService().getLocation(str);
	}
	...
	private static final String[] countryName = { "N/A", "Asia/Pacific Region", "Europe", "Andorra",
			...
			"Morocco", "Monaco", "Moldova, Republic of", "Madagascar", "Marshall Islands", "Macedonia", "Mali",
			...
			"Poland", "Saint Pierre and Miquelon", "Pitcairn Islands", "Puerto Rico", "Palestinian Territory",
			"Portugal", "Palau", "Paraguay", "Qatar", "Reunion", "Romania", "Russian Federation", "Rwanda",
	 ...
	 */
	GeoIP(byte[] dbbuffer) throws IOException {
		this.dbbuffer = dbbuffer;
		init();
	}

	private void readFully(ByteArrayInputStream bis, byte[] delim) {
		for (int i = 0; i < delim.length; i++) {
			delim[i] = (byte) bis.read();
		}
	}
	...
	private void init() throws IOException {
		...
		ByteArrayInputStream bis = new ByteArrayInputStream(dbbuffer);
		bis.mark(0);
		bis.skip(dbbuffer.length - 3);
		for (i = 0; i < STRUCTURE_INFO_MAX_SIZE; i++) {
			readFully(bis, delim);
			if (delim[0] == -1 && delim[1] == -1 && delim[2] == -1) {
				databaseType = (byte) bis.read();
				if (databaseType >= 106) {
				...
				long pos = dbbuffer.length - bis.available();
				bis.reset();
				bis.skip(pos - 4);
			}
		}
